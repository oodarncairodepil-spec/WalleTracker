...
import { Upload, FileImage, Trash2, Eye, Download, RefreshCw, Wallet, Tag } from 'lucide-react'
...
export function AIParser({ isOpen, onClose, onTransactionsExtracted }: AIParserProps) {
  const { user } = useAuth()
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (!Array.isArray(result.data)) {
          console.error('OpenAI returned non-array data:', result.data)
          toast.error('Invalid response format from AI service. Expected an array of transactions.')
          return
        }
        
        // Check if we have any transactions
        if (result.data.length === 0) {
          toast.warning('No transactions were found in the image.')
          setExtractedData([])
        ...
        const errorMessage = result.record?.error_message || 'Failed to extract transactions from image'
        toast.error(errorMessage)
        
        // Still refresh history to show the error record
        await loadParsingHistory()
      }
    } catch (error) {
  ...
  const handleUseTransactions = () => {
    if (Array.isArray(extractedData) && onTransactionsExtracted) {
  ...
  const downloadJSON = () => {
    ...
    const exportFileDefaultName = `ai-extracted-transactions-${new Date().toISOString().split('T')[0]}.json`
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-4 max-h-96 overflow-y-auto">
                        {Array.isArray(extractedData) && extractedData.map((transaction, index) => (
